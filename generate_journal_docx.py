...
def set_cell_bg(cell, hex_color: str):
    """Fond de couleur sur une cellule de tableau."""
    tc   = cell._tc
...
def set_col_widths(table, widths_cm: list):
    """Fixe la largeur de chaque colonne en cm."""
    for row in table.rows:
        for i, cell in enumerate(row.cells):
            if i < len(widths_cm):
                ...
                emu  = int(widths_cm[i] * 360000)  # cm → EMU → twips
...
def add_colored_border_top(table, hex_color: str, size=12):
    """Ajoute un filet coloré en haut du tableau."""
    tbl  = table._tbl
    tblPr = tbl.find(qn("w:tblPr"))
    if tblPr is None:
        tblPr = OxmlElement("w:tblPr"); tbl.insert(0, tblPr)
...
def p_run(para, text: str, bold=False, italic=False, size_pt=10,
          color: RGBColor = INK, font="Times New Roman"):
...
def add_para(cell_or_doc, text: str, bold=False, italic=False, size_pt=10,
             color: RGBColor = INK, align=WD_ALIGN_PARAGRAPH.LEFT,
             space_before=0, space_after=2, font="Times New Roman") -> None:
    p = cell_or_doc.add_paragraph()
    p.alignment = align
    p.paragraph_format.space_before = Pt(space_before)
    p.paragraph_format.space_after  = Pt(space_after)
    p.paragraph_format.line_spacing = Pt(size_pt * 1.35)
    p_run(p, text, bold=bold, italic=italic, size_pt=size_pt, color=color, font=font)

def add_rule(cell_or_doc, color_hex="C0B8A8", size=4):
    """Filet horizontal via paragraphe avec bordure basse."""
    p   = cell_or_doc.add_paragraph()
...
def add_article(cell, art: dict, title_size=13, show_img=True, img_w_cm=5.5, img_h_cm=3.2):
    """Insère un article complet dans une cellule."""
    # Tag rubrique
    add_red_tag(cell, rub_label(art))

    # Image (centrée, taille uniforme)
    if show_img:
        url = art.get("imgUrl","")
...
def add_une_header(doc, edition_date: datetime, numero: int):
    """En-tête page 1 : bande bleue avec logo."""
...
def add_inner_header(doc, rubrique_label: str, edition_date: datetime, numero: int):
    """En-tête compact pages intérieures."""
    tbl = doc.add_table(rows=1, cols=3)
    remove_table_borders(tbl)
    add_colored_border_top(tbl, "C8302A", size=12)
    set_cell_bg(tbl.cell(0,0), "0E2B62")
    set_cell_bg(tbl.cell(0,1), "0E2B62")
    set_cell_bg(tbl.cell(0,2), "0E2B62")
...
def add_footer_line(doc, page_num: int, edition_date: datetime):
    """Pied de page simulé en bas de section."""
    tbl = doc.add_table(rows=1, cols=3)
    remove_table_borders(tbl)
...
def add_section_break(doc):
    """Saut de page pour séparer les pages du journal."""
    p = doc.add_paragraph()
    pPr = p._p.get_or_add_pPr()
...
def build_une(doc, articles: list, edition_date: datetime, numero: int):
    ...
    # ── Article vedette : 2 colonnes (texte | image) ──────────────────────────
    if vedette:
    ...
    # ── Articles secondaires : 3 colonnes ─────────────────────────────────────
    if secondaires:
...
IMG_W_COL3 = 5.5   # largeur image en 3 colonnes (cm)
IMG_H_COL3 = 3.2   # hauteur image uniforme (cm)
IMG_W_COL2 = 8.5
IMG_H_COL2 = 4.5
...
def main():
    ...
    # ── Mise en page A4, marges serrées ──
